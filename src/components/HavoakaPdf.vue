<template>
  ...
  genererPdf(){
    ...
    pdf.save("Lisitra_Mpitondra_MICRO.pdf");
                            ...
                            <!-- GenererPdf methode 2


  genererPdf(){
    // Capture le rendu HTML
html2canvas(document.querySelector(".MitondraMicro"),{ scale: 1 }).then(canvas => {
// Crée une instance de jsPDF
let pdf = new jsPdf("l", "pt", "a3");

// Calcul de la ratio de l'échelle
let ratio = canvas.width / canvas.height;

// Ajoute le rendu HTML capturé comme image dans le PDF
pdf.addImage(canvas.toDataURL("image/png"), "PNG", 0, 0, 1200, 1170 / ratio);
